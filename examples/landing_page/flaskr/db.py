...
def get_db():
    """
    g is a special object that is unique for each request. It is used to store data that might be accessed
    by multiple functions during the request. The connection is stored and reused instead of
...
# Following code is from the tutorials and should initialize the query_store. However, we do not need to do this
# because we will use the one from the rdf_data_citation package
